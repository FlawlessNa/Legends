...
class Controller(InputHandler):
    ...
    @functools.lru_cache
    def key_binds(self, ign: str) -> dict[str, str]:
        """
    ...
    async def move(
        ...
        await self._focused_input(handle, direction, ["keydown"], enforce_delay=False)
        ...
        try:
            ...
            if jump:
                await self._non_focused_input(
                    handle, self.key_binds["jump"], [win32con.WM_KEYUP], cooldown=0
                )
