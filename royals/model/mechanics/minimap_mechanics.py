            ...
            for idx, (conn, conn_type) in enumerate(
                ...
                conn_list.append(
                    (
                        conn.x,
                        conn.y,
                        MinimapConnection.convert_to_string(
                            self.connections_types[idx]
                        ),
                    )
                )
        return (
...
class MinimapFeature(Box):
    ...
    def __iter__(self):
        ...
        if self.is_platform and not self.is_irregular:
            ...
            y_range = (
                range(self.top, self.bottom + 1)
                if not self.backward
                else range(self.bottom, self.top - 1, -1)
            )
            if len(x_range) > len(y_range):
                y_values = np.interp(
                    np.linspace(0, len(y_range) - 1, len(x_range)),
                    np.arange(len(y_range)),
                    y_range,
                )
                ...
                x_values = np.interp(
                    np.linspace(0, len(x_range) - 1, len(y_range)),
...
class MinimapPathingMechanics(BaseMinimapFeatures, Minimap, ABC):
    ...
    def _preprocess_img(self, image: np.ndarray) -> np.ndarray:
        ...
        for feature in self.features.values():
            ...
            cv2.line(canvas, pt1, pt2, (255, 255, 255), 1, lineType=cv2.LINE_4)
