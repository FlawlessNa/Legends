...
class KerningLine1Area1(BaseMap):
	...
	def first_ladder_bot_to_main(self) -> Box:
		return Box(left=31, right=31, top=48, bottom=55, name='first_ladder_bot_to_main')

	@cached_property
	def second_ladder_bot_to_main(self) -> Box:
		return Box(left=97, right=97, top=48, bottom=56, name='second_ladder_bot_to_main')

	@cached_property
	def third_ladder_bot_to_main(self) -> Box:
		return Box(left=126, right=126, top=48, bottom=56, name='third_ladder_bot_to_main')

	@cached_property
	def fourth_ladder_bot_to_main(self) -> Box:
		return Box(left=198, right=198, top=48, bottom=56, name='fifth_ladder_bot_to_main')
	...
	def platform_4_right_ladder(self) -> Box:
		return Box(left=174, right=174, top=25, bottom=36, name='platform_4_right_ladder')

	@cached_property
	def platform_4_left_ladder(self) -> Box:
		return Box(left=157, right=157, top=26, bottom=39, name='platform_4_left_ladder')
	...
	def safe_spot_right_ladder(self) -> Box:
		return Box(left=217, right=217, top=29, bottom=40, name='safe_spot_right_ladder')
	...
	def platform_9_right_ladder(self) -> Box:
		return Box(left=168, right=169, top=40, bottom=47, name='platform_9_right_ladder')

	@cached_property
	def platform_9_left_ladder(self) -> Box:
		return Box(left=140, right=140, top=40, bottom=49, name='platform_9_left_ladder')
