...
class AbilityMenu(InGameDynamicVisuals):
    ...
    """
    _menu_icon_detection_needle: np.ndarray = cv2.imread(os.path.join(ROOT, 'royals/assets/detection_images/ability_menu.png'))

    ign_box: Box = Box(offset=True, name='IGN', left=54, right=-5, top=26, bottom=28, config='--psm 7')
    ...
    str_box: Box = Box(offset=True, name='STR', left=54, right=-20, top=223, bottom=223, config=f'--psm 7 -c tessedit_char_whitelist={string.digits}+()')
    dex_box: Box = Box(offset=True, name='DEX', left=54, right=-20, top=242, bottom=242, config=f'--psm 7 -c tessedit_char_whitelist={string.digits}+()')
    int_box: Box = Box(offset=True, name='INT', left=54, right=-20, top=260, bottom=260, config=f'--psm 7 -c tessedit_char_whitelist={string.digits}+()')
    luk_box: Box = Box(offset=True, name='LUK', left=54, right=-20, top=278, bottom=278, config=f'--psm 7 -c tessedit_char_whitelist={string.digits}+()')
    add_str_box: Box = Box(offset=True, name='AddSTR', left=150, right=-8, top=228, bottom=222)
    add_dex_box: Box = Box(offset=True, name='AddDEX', left=150, right=-8, top=247, bottom=241)
    add_int_box: Box = Box(offset=True, name='AddINT', left=150, right=-8, top=265, bottom=259)
    add_luk_box: Box = Box(offset=True, name='AddLUK', left=150, right=-8, top=283, bottom=277)
    ...
    ability_points_box: Box = Box(offset=True, name='AP', left=91, right=-49, top=197, bottom=193, config=f'--psm 7 -c tessedit_char_whitelist={string.digits}')
...
if __name__ == '__main__':
    from botting.utilities import take_screenshot, client_handler
    from royals import royals_ign_finder
    import cv2
    HANDLE = client_handler.get_client_handle('FarmFest1', royals_ign_finder)
    test = AbilityMenu()
    menu_box = test._menu_icon_position(HANDLE)
    img = take_screenshot(HANDLE, menu_box + test.ability_points_box)
    cv2.imshow('test', img)
    cv2.waitKey(0)
