...
def randomize_params(*args,
    ...
    def outer(func: callable) -> callable:
        ...
        def inner(*func_args, **func_kwargs):
            arguments_mapper: dict = inspect.getcallargs(func, *func_args, **func_kwargs)
            ...
            for arg in itertools.chain(params_to_randomize, ignore_args):
                assert arg in arguments_mapper, f"The specified argument {arg} is not part of the specified keyword arguments in the underlying function {func.__qualname__}"

            if abs_threshold:
                randomized_params = {k: random.uniform(v - abs_threshold, v + abs_threshold) if isinstance(v, (int, float)) else v
                                     for k, v in arguments_mapper.items() if k in params_to_randomize}
            else:
                randomized_params = {k: random.uniform(v * (1 - perc_threshold), v * (1 + perc_threshold)) if isinstance(v, (int, float)) else v
                                     for k, v in arguments_mapper.items() if k in params_to_randomize}

            # For Box parameters, return a random point within their box areas instead
            randomized_params = {k: v.random_point() if isinstance(v, Box) else v for k, v in randomized_params.items()}

            if enforce_sign:
                for k, v in randomized_params.items():
                    if isinstance(v, (int, float)) and v * arguments_mapper[k] < 0:
                        raise ValueError(f"The randomized value of {k} has a different sign than the original value. This is not allowed when enforce_sign=True")

            for k, v in convert_args.items():
                assert k in randomized_params.keys(), f"The specified argument {k} from convert_args is not among the parameters to be randomized {params_to_randomize}."
                assert v in ['int', 'float'], f"Each val. in convert_args dict should either be equal to 'int' or 'float'"
