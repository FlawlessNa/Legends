...
ANNOTATOR_OUTPATH = os.path.join(ROOT, 'data/character_detection_images/ChronosClericTraining')

# Make sure to Ctrl+R the paths within the txt files to the images folder
TRAIN_TXT_FILE = os.path.join(ROOT, 'data/character_detection_images/ChronosClericTraining/train.txt')
...
for line in lines:
    ...
    if line in train_lines:
        new_path = os.path.join(ANNOTATOR_OUTPATH, 'train', 'images', os.path.basename(full_path))
        new_label_path = os.path.join(ANNOTATOR_OUTPATH, 'train', 'labels', os.path.basename(label_path))
    elif line in val_lines:
        new_path = os.path.join(ANNOTATOR_OUTPATH, 'val', 'images', os.path.basename(full_path))
        new_label_path = os.path.join(ANNOTATOR_OUTPATH, 'val', 'labels', os.path.basename(label_path))
    else:
        new_path = os.path.join(ANNOTATOR_OUTPATH, 'test', 'images', os.path.basename(full_path))
        new_label_path = os.path.join(ANNOTATOR_OUTPATH, 'test', 'labels', os.path.basename(label_path))
