...
class TestDiscordIO(unittest.IsolatedAsyncioTestCase):
    ...
    async def retrieves_specified_chat_channel(
        self, mock_config_reader, mock_get_all_channels
    ):
        ...
        mock_get_all_channels.return_value = [
            MagicMock(id=123, name="test_channel", type=MagicMock(name="text"))
        ]
        pipe = MagicMock()
    ...
    @patch("botting.communications.discord_comm.discord.Client.get_all_channels")
    @patch("botting.utilities.config_reader")
    async def returns_none_if_no_specified_chat_channel(
        self, mock_config_reader, mock_get_all_channels
    ):
        mock_config_reader.return_value = {
            "DEFAULT": {"user": "test_user"},
    ...
    async def triggers_on_ready_callback(self, mock_config_reader):
        mock_config_reader.return_value = {"DEFAULT": {"user": "test_user"}}
        pipe = MagicMock()
        discord_io = DiscordIO(pipe)
        with patch(
            "botting.communications.discord_comm.logger.info"
        ) as mock_logger_info:
            await discord_io.on_ready()
            mock_logger_info.assert_called_with(
                f"Discord Communication Established with {discord_io.user}."
            )
    ...
    async def processes_valid_messages(self, mock_config_reader):
        ...
        msg = MagicMock(
            ...
            channel=MagicMock(name="test_channel"),
    ...
    async def ignores_invalid_messages(self, mock_config_reader):
        ...
        discord_io.user = MagicMock(id=456)
        msg = MagicMock(
            author=MagicMock(id=789),
            channel=MagicMock(name="wrong_channel"),
            content="test_message",
        )
        with patch(
            "botting.communications.discord_comm.logger.info"
    ...
    async def relays_image_message_to_discord(self, mock_config_reader):
        ...
        discord_io.get_channel = MagicMock(return_value=AsyncMock(send=AsyncMock()))
        with patch(
            "botting.communications.discord_comm.cv2.imwrite"
        ) as mock_imwrite, patch(
            "botting.communications.discord_comm.os.remove"
        ) as mock_remove, patch(
            "builtins.open", new_callable=MagicMock
        ) as mock_open:
            await discord_io.relay_main_to_disc()
            mock_imwrite.assert_called_with(
                "temp.png", np.zeros((10, 10, 3), dtype=np.uint8)
            )
